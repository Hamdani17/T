...
@sofethon.on(events.NewMessage(outgoing=True, pattern=r"\.الصيد"))
async def _(event):
    await event.edit(
        '''
**-- -- -- -- -- -- -- -- --
...
@sofethon.on(events.NewMessage(outgoing=True, pattern=r"\.الانواع"))
async def _(event):
    if ispay2[0] == "yes":
...
@sofethon.on(events.NewMessage(outgoing=True, pattern=r"\.صيد"))
        ...
        try:
            ch = await sofethon(
                functions.channels.CreateChannelRequest(
                    title="SoFe Source",
                    about="This channel to hunt username by - @x_xxi ",
                )
            )
            ch = ch.updates[1].channel_id
            await event.edit(f"**تم انشاء القناة بنجاح .. سيتم صيد نوع {choice} !**")
        except Exception as e:
            await sofethon.send_message(
...
@sofethon.on(events.NewMessage(outgoing=True, pattern=r"\.تثبيت"))
async def _(event):
    msg = event.text.split()
    try:
        ch = str(msg[1])
    ...
    isclaim.clear()
    isclaim.append("off")
    await sofethon.send_message(event.chat_id, "**- تم الانتهاء من التثبيت بنجاح**")


@sofethon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة الصيد"))
async def _(event):
    if "on" in isclaim:
...
@sofethon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة التثبيت"))
async def _(event):
    if "on" in isauto:
        ...
        await event.edit("-لقد حدث خطأ ما وتوقف الامر لديك")
@sofethon.on(events.NewMessage(outgoing=True, pattern=r"\.ت (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        trys = 0
        ...
        if msg[0] == "تلقائي":  # تثبيت تلقائي عدد يوزر قناة
            ...
            @sofethon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة ت "))
            async def _(event):
                if "on" in isauto:
            ...
            for i in range(int(msg[0])):
                ...
                if "Available" in isav:
                    try:
                        await sofethon(functions.channels.UpdateUsernameRequest(
                            channel=ch, username=username))
...
⎱ By : @x_xxi **
    ''')
                        ...
                        await sofethon.send_message(event.chat_id, f'''خطأ مع {username}
    الخطأ :
    {str(eee)}''')
            ...
            isclaim.clear()
            isclaim.append("off")
            await sofethon.send_message(event.chat_id, "تم الانتهاء من التثبيت التلقائي")
