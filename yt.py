...
async def _(event):
    ...
    if j == user_id[0]:
        ...
        if data == b'F1':
            ...
            await sofethon.send_file(event.chat_id, file)
            os.remove(file)
        if data == b'F2':
...
@sofethon.on(events.NewMessage(outgoing=True, pattern="\.بحث (.*)"))
async def _(event):
    ...
    res = await sofethon.inline_query(f"@{BOT_USERNAME}", 'search')
